...
def summarize_rollup(samples: pd.DataFrame, group: str) -> Dict[str, object]:
	row = summarize_samples(samples, group, "ALL_TRAJECTORIES", (float("nan"), float("nan")))
...
def process_flight(
	...
	samples_path = group_out / f"{flight_name}_10s_heading_magnitude_samples.csv"
	samples.to_csv(samples_path, index=False)
	summary = summarize_samples(samples, group, flight_name, window)
...
def save_outputs(
	...
	flight_summary_path = output_dir / "31st_july_10s_heading_magnitude_summary_by_flight.csv"
	rollup_summary_path = output_dir / "31st_july_10s_heading_magnitude_summary_by_group.csv"
	cleaning_summary_path = output_dir / "31st_july_10s_velocity_cleaning_summary.csv"
...
def parse_arguments() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description=(
			"Run all 31st July AI/RAW trajectories through the 10s evaluation windows, "
			"clean velocity to +/-8 m/s, and calculate heading plus magnitude metrics."
		),
	)
	...
	parser.add_argument(
		"--out_dir",
		default="31st_July_heading_magnitude_10s",
